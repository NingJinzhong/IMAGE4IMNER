...
from chinese_utils import normalize_chinese_number
import random
    ...
    def trans(self, text):
        ...
        for word in tokens:
            if word in self.phone_map:
                ...
                for i in range(len(self.phone_map[word].split(' '))):
                    if len(word)>1:
                        chars.append(random.choice(list(word)))
                    else:
                        chars.append(word)
            elif len(word) > 1:
                for char in word:
                    if char in self.phone_map:
                        ...
                        for i in range(len(self.phone_map[char].split(' '))):
                            chars.append(char)
        return ' '.join(phones),chars
