...
from modelscope.models.multi_modal.ofa.generate import utils
from modelscope.models.multi_modal.ofa.modeling_ofa import (Embedding, OFADecoder, OFAModel, OFAPreTrainedModel,
                           _expand_mask)
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < context_layer:# and (~padding_mask).any() is False:
                # phone_x = phone_x.transpose(0, 1)
                # phone_x, (z_, lr_) = layer(phone_x, self_attn_padding_mask=phone_padding_mask, need_weights=False)
                # phone_x = phone_x.transpose(0, 1)
                pass
            ...
            if i == context_layer and phone_x is not None and phone_x_conv is not None:
                x = x.transpose(0, 1)
                x = x + x_modalflagemb
                phone_x = phone_x + phone_modalflagemb
...
class MMSpeechEncoder(MMSpeechPreTrainedModel):
    ...
    def __init__(self,
        ...
        self.modalflagtypeembedding = nn.Embedding(2,self.embed)
        ...
        self.embed_tokens = embed_tokens#text embeding
        ...
        self.audioflagproj =nn.Linear(2*self.embed,self.embed) 
        self.textflagproj =nn.Linear(2*self.embed,self.embed)
        self.relu = nn.ReLU()
    ...
    def forward(self,
                ...
                char_items: Optional[torch.Tensor] = None,
        ...
        if mask:
            ...
            x = features
            mask_indices = None
        audioflagid = fbank_masks.unsqueeze(1).repeat(1,x.shape[1]).long()
        audioflag_embeding =  self.modalflagtypeembedding(audioflagid)

        x = audioflag_embeding + x
        # cattemp = torch.cat([audioflag_embeding,x],dim=-1)
        # x = torch.relu(self.audioflagproj(cattemp))
        ...
        if phone_items is not None:
            ...
            char_x = self.embed_tokens(char_items)
            phone_padding_mask = phone_items.eq(self.phone_padding_idx)
            phoneflagid = phone_masks.unsqueeze(1).repeat(1,phone_x.shape[1]).long()
            phoneflag_embeding = self.modalflagtypeembedding(phoneflagid)
            phone_x = phone_x+char_x+phoneflag_embeding
            # cattemp = torch.cat([phoneflag_embeding,phone_x],dim=-1)
            # phone_x = torch.relu(self.textflagproj(cattemp))
        ...
        x, layer_results, pos_embed, padding_mask = self.encoder(
            ...
            x_modalflagemb = torch.zeros_like(audioflag_embeding),
            phone_x=phone_x,
            phone_padding_mask=phone_padding_mask,
            phone_modalflagemb = torch.zeros_like(phoneflag_embeding),
            layer=layer,
            context_layer=6)
